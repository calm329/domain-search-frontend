...
const Search = () => {  
    ...
    /**  
     * Generate affiliate link with domain appended as a query parameter  
     */  
    const generateAffiliateLink = (baseUrl, domain) => {  
        let url;  
        switch (baseUrl) {  
            case 'namecheap':  
                url = `https://www.namecheap.com/domains/registration/results/?domain=${domain}.com`;  
                break;  
            case 'dreamhost':  
                url = `https://www.dreamhost.com/domains/?domain=${domain}.com`;  
                break;  
            case 'a2hosting':  
                url = `https://my.a2hosting.com/cart.php?domain=register&a=add&sld=${domain}&tld=.com`;  
                break;  
            case 'cloudways':  
                url = `https://www.cloudways.com/en/?id=1755288&domain=${domain}.com`;  
                break;  
            default:  
                url = "#";  
        }  
                        ...
                        <div className='p-5 border border-[#6feec7] border-opacity-25 flex justify-between items-center' key={index}>  
                            ...
                            <div className='space-x-3'>  
                                {mainDomain.available ?  
                                    <a href='.tkqlhce.com/click-100703940-15083053' target="_blank" rel="noreferrer">  
                                            ...
                                            <DialogContent className="bg-neutral-900 text-neutral-100 max-w-2xl w-full border-neutral-700 ring-0 px-4 py-6 mx-auto">  
                                                ...
                                                {domainStatus?.available && (  
                                                    <DialogFooter className="flex flex-col space-y-3">  
                                                        <a href={generateAffiliateLink('namecheap', currentDomain)} target="_blank" rel="noreferrer" className="w-full">  
                                                            <button className='bg-[#6feec7] rounded-sm px-5 py-2 text-[#2A2A2A] w-full'>Get Here on Namecheap</button>  
                                                        </a>  
                                                        <a href={generateAffiliateLink('dreamhost', currentDomain)} target="_blank" rel="noreferrer" className="w-full sm:space-x-0">  
                                                            <button className='bg-[#6feec7] rounded-sm px-5 py-2 text-[#2A2A2A] w-full'>Best for Bloggers - Dreamhost</button>  
                                                        </a>  
                                                        <a href={generateAffiliateLink('a2hosting', currentDomain)} target="_blank" rel="noreferrer" className="w-full mx-0">  
                                                            <button className='bg-[#6feec7] rounded-sm px-5 py-2 text-[#2A2A2A] w-full'>Best for Affiliates - A2 Hosting</button>  
                                                        </a>  
                                                        <a href={generateAffiliateLink('cloudways', currentDomain)} target="_blank" rel="noreferrer" className="w-full mx-0">  
